 ...
 * - password: User's password. Required and must match a pattern enforcing 8-32 characters, at least one uppercase letter, one lowercase letter, one number, and one special character. Validation only runs if the password is new or modified.
 ...
 * - resetToken: String used for password reset operations.
 * - resetTokenExpires: Date indicating when the reset token expires.
 *
 * Middleware:
    ...
    password: {
        ...
        validate: {
            validator: function (val) {
                ...
                return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_\-+=\[\]{};':"\\|,.<>/?]).{8,32}$/.test(val);
            },
